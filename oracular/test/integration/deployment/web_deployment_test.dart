...
void main() {
  group('Web Build and Deployment Tests', () {
    ...
    setUp(() async {
      tempDir = await createDeploymentTempDir('oracular_deploy_');
    ...
    group('Flutter Web Build', () {
      test(
        ...
        () async {
          ...
          // Run flutter pub get
          ProcessResult pubResult = await runner.run('flutter', <String>[
            'pub',
            'get',
          ...
          // Build web
          ProcessResult buildResult = await runner.run('flutter', <String>[
            'build',
            'web',
            '--release',
          ], workingDirectory: projectPath);
          ...
          final Directory buildDir = Directory(
            p.join(projectPath, 'build', 'web'),
          );
          expect(
            buildDir.existsSync(),
            isTrue,
            reason: 'Build output should exist',
          );
    ...
    group('Jaspr Web Build', () {
      test(
        ...
        () async {
          ...
          // Build with jaspr
          ProcessResult buildResult = await runner.run('jaspr', <String>[
            'build',
          ], workingDirectory: projectPath);
          ...
          // Verify build output exists
          final Directory buildDir = Directory(
            p.join(projectPath, 'build', 'jaspr'),
          );
          expect(
            buildDir.existsSync(),
            isTrue,
...
''');
          ...
          expect(
            ...
            equals(0),
            reason:
                'Firebase hosting deploy should succeed:\n${deployResult.stdout}\n${deployResult.stderr}',
          );
      ...
      test(
        ...
        () async {
          ...
          expect(
            buildSuccess,
            isTrue,
            reason: 'FirebaseService.buildWeb should succeed',
          );
