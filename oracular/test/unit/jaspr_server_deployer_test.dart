...
/// scripted [ProcessResult]s in order. Mirrors `_CapturingRunner` in
/// `server_setup_test.dart` so the two deploy pipelines (arcane_server
/// and the Jaspr SSR/hybrid binary) share a consistent testing shape.
  ...
  group('JasprServerDeployer.deploy', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp
          .createTemp('oracular_jaspr_deployer_');
    });
    ...
    Future<void> scaffoldJaspr(SetupConfig cfg) async {
      ...
      await File(p.join(webPath, 'Dockerfile.jaspr'))
    ...
    ProcessResult gitNotARepo() =>
        ProcessResult(exitCode: 128, stdout: '', stderr: 'not a git repository');

    test('returns failure when render mode does not require Cloud Run', () async {
      ...
      final JasprServerDeployer deployer =
          JasprServerDeployer(cfg, runner: runner);
    ...
    test(
      'happy path runs preflight → build → push → deploy in order '
      'and returns the Cloud Run URL',
      () async {
        final SetupConfig cfg = configFor();
        await scaffoldJaspr(cfg);

        const String cloudRunUrl =
            'https://demo-app-web-abc123-uc.a.run.app';
        ...
        final List<String> buildArgs =
            runner.invocations.firstWhere((List<String> inv) =>
                inv.first == 'docker' && inv.contains('build'));
        expect(buildArgs, containsAllInOrder(<String>[
          'docker',
          'build',
          '--platform',
          'linux/amd64',
    ...
    test('aborts on docker build failure without attempting push', () async {
      ...
      final bool sawPush = runner.invocations.any((List<String> inv) =>
          inv.first == 'docker' && inv.length > 1 && inv[1] == 'push');
      expect(sawPush, isFalse);
    });

    test('aborts on docker push failure without attempting gcloud run deploy',
        () async {
      final SetupConfig cfg = configFor();
      await scaffoldJaspr(cfg);
      ...
      final bool sawRunDeploy = runner.invocations.any((List<String> inv) =>
          inv.first == 'gcloud' &&
          inv.length > 2 &&
          inv[1] == 'run' &&
          inv[2] == 'deploy');
    ...
    test(
      'respects custom jasprServerServiceName (and rewrites underscores '
      'to dashes for Cloud Run naming)',
      () async {
        final SetupConfig cfg =
            configFor(serviceName: 'custom_marketing_web');
        await scaffoldJaspr(cfg);

        const String url =
            'https://custom-marketing-web-xyz789-uc.a.run.app';
        final _CapturingRunner runner = _CapturingRunner(<ProcessResult>[
          ...preflightSuccesses(),
          gitNotARepo(),
          ProcessResult(exitCode: 0, stdout: '', stderr: ''),
        ...
        expect(
          result.imageTag,
          contains('/custom-marketing-web:latest'),
        );
      },
    );

    test('preflight short-circuit on wrong-project SA produces clear failure',
        () async {
      ...
      final bool dockerInvoked =
          runner.invocations.any((List<String> inv) => inv.first == 'docker');
